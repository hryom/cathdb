  ...
  <xsl:choose>
    <xsl:when test="contains($template, '%')">
      ...
      <xsl:variable name="rest">
        <xsl:choose>
          <xsl:when test="$candidate = 'n' and @label and @label = '' and substring(substring-after($template, '%'), 2, 1) = '.'">
            <xsl:value-of select="substring($template, string-length(substring-before($template, '%'))+4)"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="substring($template, string-length(substring-before($template, '%'))+3)"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:variable>
